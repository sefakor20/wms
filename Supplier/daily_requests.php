...
$pagination = $fetch_data->getTwoFieldsTotal('Warehouse_Stock_Request', 'supplier_id', $_SESSION['sup_id'], 'request_status', 1);
...
$all_requests = $fetch_data->getMultipleDataCompare("SELECT Warehouse_Stock_Request.id, Item.name AS item, Category.name AS category, quantity, Request_Status.name AS r_status, Warehouse_Stock_Request.created_at", 'Warehouse_Stock_Request', "JOIN Item ON Item.id = Warehouse_Stock_Request.item_id JOIN Category ON Category.id = Warehouse_Stock_Request.category_id JOIN Request_Status ON Request_Status.id = Warehouse_Stock_Request.request_status", 'supplier_id', $_SESSION['sup_id'], 'request_status', 1, $rows, $offset);
...
<head>
    ...
    <title>Supplier - Pending Requests</title>
