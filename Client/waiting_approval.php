...
$pagination = $fetch_data->getTwoFieldsTotal('Warehouse_Release_Stock', 'client_id', $_SESSION['client_id'], 'status', 1);
...
$items = $fetch_data->getTwoFieldsCompareJoinLimit("SELECT Warehouse_Release_Stock.id, Item.name AS item_name, item_id, Category.name AS category_name, category_id, released_quantity, Warehouse_Release_Stock.created_at", 'Warehouse_Release_Stock', "JOIN Item ON Item.id = Warehouse_Release_Stock.item_id JOIN Category ON Category.id = Warehouse_Release_Stock.category_id", "Warehouse_Release_Stock.status", 1, 'client_id', $_SESSION['client_id'], $rows, $offset);
...
<head>
	...
	<title>Client - Stock Waiting Approval</title>
                                ...
                                <table class="table table-hover">
                                    ...
                                    <?php foreach($items as $item): ?>
                                        <tr>
                                            ...
                                            <a href="../Submits/client_approve.php?wrs_id=<?php echo $item->id; ?>&client_id=<?php echo $_SESSION['client_id']; ?>&item_id=<?php echo $item->item_id; ?>&category_id=<?php echo $item->category_id; ?>&released_quantity=<?php echo $item->released_quantity; ?>">Approve Stock</a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                                    </tbody>
